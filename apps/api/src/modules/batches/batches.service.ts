...
export class BatchesService {
   ...
   *
   * Es un upsert por (batchId, label): cambiar el equipo de una etiqueta ya
  ...
  async assignInstrument(
    ...
    await this.findById(batchId, organizationId)
    ...
    await this.prisma.batchInstrumentAssignment.upsert({
      where: { batchId_label: { batchId, label: data.label } },
      create: {
        batchId,
        label: data.label,
        order: data.order,
    ...
    return this.findById(batchId, organizationId)
  }

  /**
   * Quita una asignacion. El id de la asignacion llega de la URL, asi que se
   * busca acotado al lote —que ya se resolvio contra la
   * organizacion—: sin eso, conocer un id alcanzaria para borrarle la
   * trazabilidad a otro laboratorio.
   */
  async unassignInstrument(batchId: string, assignmentId: string, organizationId: string) {
    await this.findById(batchId, organizationId)

    const asignacion = await this.prisma.batchInstrumentAssignment.findFirst({
      where: { id: assignmentId, batchId },
      select: { id: true },
    })
    if (!asignacion) throw new NotFoundException('Asignacion no encontrada')

    await this.prisma.batchInstrumentAssignment.delete({ where: { id: assignmentId } })
