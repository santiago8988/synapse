...
async function main() {
  ...
  // Crear registro "Instrumentos" (sistema) — pilot del colapso de companions
  // (Records-as-Lists). Reemplaza la antigua tabla Instrument: el "instrumento"
  // es ahora una Entry de este Record, con field DROPDOWN ESTADO + isStatus.
  const existingInstr = await prisma.record.findFirst({
    where: { organizationId: org.id, name: 'Instrumentos', isSystem: true },
  })
  if (!existingInstr) {
    const orgUserInstr = await prisma.organizationUser.findFirst({
      where: { organizationId: org.id },
    })
    const createdByIdInstr = orgUserInstr?.id || 'system'

    await prisma.record.create({
      data: {
        organizationId: org.id,
        createdById: createdByIdInstr,
        name: 'Instrumentos',
        type: 'INSTRUMENTAL',
        status: 'ACTIVE',
        isSystem: true,
    ...
    console.log('Instrumentos (record sistema) creado')
  } else {
    console.log('Instrumentos (record sistema) ya existe')
  }
